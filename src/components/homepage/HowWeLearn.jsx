    ...
    >
      <ul className="grid grid-cols-1 grid-rows-4 md:grid-rows-2 md:grid-cols-2 lg:grid-cols-4 lg:grid-rows-1 gap-6">
        {learnData.map((l, index) => (
          <li
            key={index}
            className="border border-gray-100 dark:border-DarkblackBorder dark:bg-DarkblackBorder dark:text-white p-6 flex flex-col items-start justify-start shadow-lg rounded-[12px] transition-all duration-300 hover:-translate-y-1.5 hover:shadow-xl"
          >
            <img
              src={l.img}
              alt={l.alt}
