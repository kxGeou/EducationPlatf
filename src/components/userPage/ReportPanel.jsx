        ...
        {["Do zrobienia", "W trakcie", "Zrobione"].map((status) => (
          ...
          >
            ...
            <div className="flex flex-col text-left">
              <span className={` text-sm font-medium ${statusStyles[status].color} ${statusStyles[status].text}`}>
                {status}
              </span>
        ...
        {!openForm ? (
                  ...
                  >
                    <div className="flex justify-between items-center">
                      <div className="flex items-center gap-2">
                        ...
                        <h3 className="font-semibold text-gray-800 dark:text-gray-200">{r.topic}</h3>
                      </div>
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${statusStyles[r.status]?.color} ${statusStyles[r.status]?.text}`}>
                        {r.status}
                      </span>
                    </div>
                    <p className="text-gray-500 dark:text-gray-300">{r.description}</p>
                  </div>
                ))
              ) : (
          ...
          >
            ...
            <div className="flex gap-4 justify-end">
              ...
              <button
                type="submit"
                className="px-4 py-2 rounded-lg bg-gradient-to-br from-primaryBlue to-secondaryBlue text-white dark:from-primaryGreen dark:to-secondaryGreen transition cursor-pointer hover:scale-[1.025]"
              >
                Wyślij zgłoszenie
              </button>
            </div>
