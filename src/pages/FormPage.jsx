...
import { User, PlusCircle, List, Sun, Moon, MinusCircle } from "lucide-react";
...
function FormPage({ isDark,setIsDark }) {
  ...
  const handleSave = () => {
    if (!title || options.some((o) => !o)) return toast.error("Uzupełnij wszystkie pola!");
  ...
  return (
    <div className="min-h-screen  bg-gray-100 dark:bg-blackText text-blackText dark:text-white" data-theme={isDark ? "dark" : "light"}>
      ...
      <div className="flex gap-4 p-4 bg-white dark:bg-DarkblackBorder shadow-sm justify-center">
        ...
        className={`flex items-center gap-2 px-4 py-2 rounded-[12px] transition ${
            activeTab === "create" ? "bg-primaryBlue text-white dark:bg-primaryGreen" : "opacity-75"
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-[12px] transition ${
            activeTab === "list" ? "bg-primaryBlue text-white dark:bg-primaryGreen" : "opacity-75"
          }`}
        >
      ...
      <div className="p-6">
        {activeTab === "create" && (
          <div className="max-w-[700px] mx-auto bg-white dark:bg-DarkblackBorder rounded-[12px] shadow-md p-6 flex flex-col gap-4">
            ...
            <div className="flex gap-2 mt-4 w-full justify-between">
              <button
                onClick={() => options.length < 6 && setOptions([...options, ""])}
        ...
        {activeTab === "list" && (
          <div className="grid md:grid-cols-2 gap-6">
            {polls.map((poll) => (
              <div key={poll.id} className="bg-white dark:bg-DarkblackBorder rounded-2xl shadow-md p-5">
